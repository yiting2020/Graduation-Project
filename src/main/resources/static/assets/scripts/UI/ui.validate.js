...
; (function () {
    ...
    //SQL关键字验证
    var sqlKeyDict = {};
    "INSERT,UPDATE,DELETE,FROM,AS,OR,AND,=,WHERE,ORDER BY,GROUP BY,ASC,DESC,DECLARE,PROC,ALTER,PROCEDURE,FUNCTION,CREATE,DROP".replace(/[^,]+/g, function (keyWord) {
        sqlKeyDict[keyWord.toLowerCase()] = 1;
    });
    function checkSqlKeyWord(value) {
        var arr = value.split(" ");
        var i = 0,
            len = arr.length;
        for(; i < len; i++) {
            if(sqlKeyDict.hasOwnProperty(arr[i].toUpperCase())) {
                return true;
            }
        }
        return false;
    }
    $.validator.addMethod("SqlKeyWord", function(value, element) {
        return this.optional(element) || !(checkSqlKeyWord(value));
    }, "不能包含SQL关键字");
    //监管平台
    $.validator.addMethod("identifyCode", function(value, element) {
        var identifyCodeReg = /^[^-]{4}-[^-]{6}-[^-]{2}-[^-]{2}-[^-]{6}$/;
        return this.optional(element) || identifyCodeReg.test(value);
    }, "长度必须为24位,格式必须为“xxxx-xxxxxx-xx-xx-xxxxxx");
    //固话
    $.validator.addMethod("linePhone", function(value, element) {
        var phone = /^((0|4|8)\d{2,3})?(-)?\d{7,8}$/;
        return this.optional(element) || phone.test(value);
    }, "请输入一个正确的固话号码");

    //手机号码
    $.validator.addMethod("cellPhone", function (value, element) {
        var mobile = /^1(3|4|5|7|8|9)\d(-)?\d{4}(-)?\d{4}$/;
        return this.optional(element) || mobile.test(value);
    }, "请输入一个正确的手机号码");

    //电话号码 可以是固话也可以是手机
    $.validator.addMethod("phone", function (value, element) {
        var mobile = /^1(3|5|7|8)\d(-)?\d{4}(-)?\d{4}$/;
    ...
    //密码复杂度
    $.validator.addMethod("pwdComplex", function(value, element) {
    ...
    //自定义验证
    $.validator.addMethod("custom1", function (value, element, param) {
        if(param && $.isFunction(param.method)) {
            return this.optional(element) || !!param.method(value, element);
        } else if ($.isFunction(param)) {
            return this.optional(element) || !!param(value, element);
